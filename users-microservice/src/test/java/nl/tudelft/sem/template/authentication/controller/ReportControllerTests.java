...
public class ReportControllerTests {
    ...
    public void getAllReportsTest() {
        ...
        when(reportService.getAllReports(token)).thenThrow(new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Only admins can access reports!"));
    ...
    public void addReportTest() {
        ...
        doThrow(new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Invalid token!")).when(reportService).addReport(any(), any());
        assertEquals(reportController.addReport(report, token).getStatusCodeValue(), 401);

        doThrow(new ResponseStatusException((HttpStatus.NOT_FOUND), "Reported user not found!")).when(reportService).addReport(any(), any());
    ...
    public void deleteReportTest() {
        ...
        doThrow(new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Only admins can access reports!")).when(reportService).remove(any(), any());
        assertEquals(reportController.deleteReport(repordId.toString(), token).getStatusCodeValue(), 401);

        doThrow(new ResponseStatusException((HttpStatus.NOT_FOUND), "The report does not exist!")).when(reportService).remove(any(), any());
